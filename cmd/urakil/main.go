...
func fileRead(fileName string)(urls []string){
...
func bitlyRequest(long_url string) {
	json := fmt.Sprintf(`{"long_url": "%s", "domain": "bit.ly"}`, long_url)
	requestBody := strings.NewReader(json)
	request, err := http.NewRequest("GET", "https://ssl-api.bitly.com/v4/shorten", requestBody)
	if err != nil {
		log.Fatal(err)
	}
	request.Header.Add("Authorization", fmt.Sprintf("Bearer %s", DEFAULT_TOKEN))
...
func parseOptions(args []string) (*options, []string, *UrakilError) { 
	...
	if opts.token == "" {
		return nil, nil,
		&UrakilError{statusCode: 3, message: "no token was given"}
	}*/
